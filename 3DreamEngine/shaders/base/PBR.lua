...
function sh:constructPixel(dream, info)
	...
	//fetch material data
	vec3 rma = Texel(tex_combined, VaryingTexCoord.xy).rgb;
...
function sh:perMaterial(dream, shader, info, material)
	...
	local def = love.graphics.newImage(love.image.newImageData(1, 1))
	local def_RMA = love.image.newImageData(1, 1)
	def_RMA:setPixel(0,0,0,0,1,1)
	local def_RMA = love.graphics.newImage(def_RMA)
	shader:send("tex_albedo", dream:get_texture("albedo") or tex.default)
	...
	shader:send("tex_combined", dream:get_texture("RMA") or def_RMA)
	...
	if info.tex_normal then
		shader:send("tex_normal", dream:get_texture("normal") or tex.default_normal)
	end

	if info.tex_emission then
		shader:send("tex_emission", dream:get_texture("emission") or def)
	end
