...
function lib.get_texture(self,name_tex)
	if name_tex == "albedo" then
		return render_texs.albedo
	elseif name_tex == "RMA" then
		return render_texs.RMA
	elseif name_tex == "normal" then
		return render_texs.normal
	elseif name_tex == "emission" then
		return render_texs.emission
	end
	return nil
end

local curr_rough = nil
local curr_metal = nil
local curr_ambie = nil
function RMA_create(x, y, r, g, b, a)
	r,g,b,a = 0,0,1,1
	local _;
	if curr_rough then r,_,_,_ = curr_rough:getPixel(x, y) end
	if curr_metal then _,g,_,_ = curr_metal:getPixel(x, y) end
	if curr_ambie then 
		_,_,b,_ = curr_ambie:getPixel(x, y) 
	else
		b = 1
	end
...
local curr_mask = nil
function mask(x, y, r, g, b, a)

	mask_px = curr_mask:getPixel(x, y)
	a = a*mask_px
...
local curr_normal = nil
function mask_normal(x, y, r, g, b, a)
	local mask_px = 1
	if curr_mask then
		mask_px = curr_mask:getPixel(x, y)
	end
	r1,g1,b1,_ = curr_normal:getPixel(x, y)
	r = r*(1-mask_px) + r1*mask_px
	g = g*(1-mask_px) + g1*mask_px
	b = b*(1-mask_px) + b1*mask_px
	a = 1
...
function lib.update_texture(self,name_tex)
	if name_tex == "albedo" then
		local canvas = love.graphics.newCanvas(dimen_texture[0], dimen_texture[1])
		love.graphics.push("all")
		local wasCanvas = love.graphics.getCanvas()
		love.graphics.setCanvas(canvas)
		love.graphics.clear(1,1,1,1)
		for i = 1,#layers do
			if layers[i].albedo[0] then
				local mask_data = layers[i].albedo[0]:clone()
				if layers[i].mask[0] then
					curr_mask = layers[i].mask[0]
					mask_data:mapPixel(mask)
				end
				love.graphics.draw(love.graphics.newImage(mask_data),0,0)
			end
		end
		love.graphics.pop()
		love.graphics.setCanvas(wasCanvas)
		render_texs.albedo = canvas
		render_texs.albedo:setWrap("repeat","repeat","repeat")
	elseif name_tex == "normal" then
		local normal_map = love.image.newImageData(dimen_texture[0], dimen_texture[1])
		normal_map:mapPixel(function(x, y, r, g, b, a) return 0.5,0.5,1,1 end)
		for i = 1,#layers do
			if layers[i].normal[0] then
				curr_mask = layers[i].mask[0]
				curr_normal = layers[i].normal[0]
				normal_map:mapPixel(mask_normal)
				render_texs.normal_data = normal_map
				render_texs.normal = love.graphics.newImage(normal_map)
			end
		end
	elseif name_tex == "RMA" then
		local canvas = love.graphics.newCanvas(dimen_texture[0], dimen_texture[1])
		...
		for i = 1,#layers do
			local img_data_RMA = love.image.newImageData(dimen_texture[0],dimen_texture[1])
			curr_rough = layers[i].roughness[0]
			curr_metal = layers[i].metallic[0]
			curr_ambie = layers[i].ao[0]
			img_data_RMA:mapPixel(RMA_create)
			if layers[i].mask[0] then
				curr_mask = layers[i].mask[0]
				img_data_RMA:mapPixel(mask)
			end
			love.graphics.draw(love.graphics.newImage(img_data_RMA),0,0)
		end
		love.graphics.pop()
		love.graphics.setCanvas(wasCanvas)
		render_texs.RMA = canvas
		render_texs.RMA:setWrap("repeat","repeat","repeat")
	elseif name_tex == "emission" then
		local canvas = love.graphics.newCanvas(dimen_texture[0], dimen_texture[1])
		love.graphics.push("all")
		local wasCanvas = love.graphics.getCanvas()
		...
		love.graphics.clear()
		for i = 1,#layers do
			if layers[i].emission[0] then
				local mask_data = layers[i].emission[0]:clone()
				if layers[i].mask[0] then
					curr_mask = layers[i].mask[0]
					mask_data:mapPixel(mask)
				end
		...
		love.graphics.pop()
		love.graphics.setCanvas(wasCanvas)
		render_texs.emission = canvas
		render_texs.emission:setWrap("repeat","repeat","repeat")
	elseif name_tex == "mask" then
